...
void enc_busy_wait(void) {
	uint8_t data = enc_mac_read(ENC_MISTAT, Bank3); // Read ESTAT register (bank 3)
	while (data & 0x01) {                 // Wait until BUSY bit clears
		data = enc_mac_read(ENC_MISTAT, Bank3);
	}
}
/*
...
uint16_t enc_phy_read(uint8_t addr) {
	...
	enc_control_write(Bank2, ENC_MICMD, 1);				//MICMD.MIIRD bit set

	enc_busy_wait();

	enc_control_write(Bank2, ENC_MICMD, 0);			//MICMD.MIIRD bit cleared

	uint16_t data_LSB = enc_mac_read(ENC_MIRDL, Bank2);
	uint16_t data_MSB = enc_mac_read(ENC_MIRDH, Bank2);
