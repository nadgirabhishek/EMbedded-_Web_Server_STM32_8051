...
_transmit_tcp_packet_PARM_2:
	.ds 2
_transmit_tcp_packet_packet_10000_82:
	.ds 3
;--------------------------------------------------------
...
;packet                    Allocated with name '_transmit_tcp_packet_packet_10000_82'
;start_address             Allocated with name '_transmit_tcp_packet_start_address_10000_83'
;end_address               Allocated with name '_transmit_tcp_packet_end_address_10001_85'
;------------------------------------------------------------
;	tcp.c:28: void transmit_tcp_packet(uint8_t *packet, uint16_t packet_size)
;	-----------------------------------------
...
_transmit_tcp_packet:
	...
	mov	r6,dph
	mov	a,dpl
	mov	dptr,#_transmit_tcp_packet_packet_10000_82
	movx	@dptr,a
	mov	a,r6
	inc	dptr
	movx	@dptr,a
	...
	inc	dptr
	movx	@dptr,a
;	tcp.c:33: if ((start_address + packet_size - 1) > 0x1FFF) {
...
;	tcp.c:35: return;
	ret
00102$:
;	tcp.c:37: uint16_t end_address = start_address + packet_size - 1;
	mov	ar4,r6
	mov	a,#0x1c
	add	a, r7
	mov	r5,a
	...
	dec	r5
00131$:
;	tcp.c:38: enc_buffer_write(packet_size, start_address, packet);
	mov	dptr,#_transmit_tcp_packet_packet_10000_82
	movx	a,@dptr
	mov	r1,a
	inc	dptr
	...
	pop	ar4
	pop	ar5
;	tcp.c:41: enc28j60_set_transmit_pointers(start_address, end_address);
...
;	tcp.c:44: enc28j60_start_transmission();
	lcall	_enc28j60_start_transmission
;	tcp.c:47: if (wait_for_transmission_complete(500)) {  // Wait up to 500 ms
	mov	dptr,#0x01f4
	lcall	_wait_for_transmission_complete
	mov	a, dpl
	jz	00107$
;	tcp.c:49: if (enc28j60_transmission_successful()) {
	lcall	_enc28j60_transmission_successful
	mov	a, dpl
	jz	00104$
;	tcp.c:50: printf("TCP request sent successfully.\n\r");
	mov	a,#___str_1
	push	acc
	mov	a,#(___str_1 >> 8)
	push	acc
	...
	ret
00104$:
;	tcp.c:52: printf("TCP transmission failed. Check error flags.\n\r");
	mov	a,#___str_2
	push	acc
	mov	a,#(___str_2 >> 8)
	push	acc
	...
	ret
00107$:
;	tcp.c:56: "Transmission timeout. ENC28J60 may not be functioning correctly.\n\r");
	mov	a,#___str_3
	push	acc
	mov	a,#(___str_3 >> 8)
	push	acc
